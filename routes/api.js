...
try {
var Joward = db.get("Joward");
} catch (e) {
	console.log('')
}

var creatorList = ['@Joward','@OmegaBarBaR','@OMEGABARBAR','@OMEGABARBAR', '@OMEGABARBAR','@OMEGABARBAR','@OMEGABARBAR'];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------Miku'+'Ojan';
        
 
 async function cekApiKey(api) {
 	ap = await fznxsec.findOne({apikey:api})
 return ap;
 }
router.get('/find', async (req, res, next) => {
    var apikey = req.query.apikey
    if (!apikey) return res.json(loghandler.notparam)
    if (apikey != 'Joward') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Joward') return res.json(loghandler.invalidKey)
...
router.get('/ytmp3', async (req, res, next) => {
	...
	if(apikeyInput != 'Joward') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://api.zeks.xyz/api/ytmp3?url=&apikey=apivinz`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
